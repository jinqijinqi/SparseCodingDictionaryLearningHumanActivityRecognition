function conf=Main_TranTest_MSRAction3D(ASInd)
%generate mesh
%Author: Jin Qi
%Date:   1/30/2014
...
JointPoint.Gamma=0.001;
JointPoint.MirrorGamma=0.67;
JointPoint.C=10; %
JointPoint.lambda=0.000001
...
conf.Precision=AveragePrecision+100.0*mean((diag(confus))'./(sum(confus)+eps));%column sum
conf.Recall=AverageRecall+100.0*mean((diag(confus))'./(sum(confus')+eps));      %row sum

title(sprintf('Confusion matrix (%.2f %% accuracy)', ...
              100 * sum(diag(confus)/conf.numTest) )) ;
print('-depsc2', [conf.resultPath num2str(i) '.ps']) ;
if exist('ASInd')
save([conf.resultPath num2str(i) 'AS' num2str(ASInd) '.mat'], 'confus', 'conf') ;
else
    save([conf.resultPath num2str(i) 'AS' '.mat'], 'confus', 'conf');
end
