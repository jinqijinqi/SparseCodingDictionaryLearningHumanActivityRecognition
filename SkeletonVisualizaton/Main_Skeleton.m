...
%%
% ImageDatabase='D:/JinQi/Database/CornellAction/CAD60/';
ImageDatabase='./Database/CAD60/';
...
%         i=7;%wear contact lens
%         i=2;%talk on the phone
%         i=6;%brushing teeth
%         i=3;%write on whiteboard
%         indset=1:10;  % reproduce tracjectory pictures for Fig 1 in our Plos one paper
%         i=indset(iind);
%         iind=iind+1;
...
%         figure(1);
        ...
        PlotJointCurve(Joint,FS);
        title([ActionName(ic(i))],'FontSize',FS);
%         set(findall(gcf,'type','text'),'FontSize',35); %setting fontsize
        printpdf(gcf,[PaperFigureDir  regexprep(ActionName{ic(i)},'\s+','')]);% save image for paper
        ClassID=ic(i);
...
function PlotJointCurve(Joint,FontSize)
% Color={'y' 'm' 'c' 'r' 'g' 'b' 'k'};
...
nJoint=size(Joint,1);
set(gca,'fontsize',FS);
% JointName={'HEAD','NECK','TORSO','LEFT SHOULDER','LEFT ELBOW','RIGHT SHOULDER',...
%     'RIGHT ELBOW','LEFT HIP','LEFT KNEE','RIGHT HIP','RIGHT KNEE','LEFT HAND','RIGHT HAND','LEFT FOOT',...
%     'RIGHT FOOT'};
JointName={'H','N','T','LS','LE','RS',...
    'RE','LH','LK','RH','RK','LH','RH','LF',...
    'RF'};
hPlot=[];
...
%     end
%      figure(1);hold on;hPlot(i)=plot3(squeeze(Joint(i,1,:)),squeeze(Joint(i,2,:)),squeeze(Joint(i,3,:)),'-','color',Color{ColorIndex});
    figure(1);hold on;hPlot(i)=plot3(squeeze(Joint(i,1,:)),squeeze(Joint(i,2,:)),squeeze(Joint(i,3,:)),'-','color',Color(i,:));
end
% gridLegend(hPlot,3,JointName);
% lh=legend(JointName{:},'FontSize',4);
% set(lh,'units','pixels');
legend(JointName,'Location','eastoutside','Orientation','vertical','FontSize',FS);
% lh=columnlegend(3,JointName,'Location','southoutside','Orientation','horizontal','LineWidth',20);
% set(lh,'units','pixels');
% LegendSize(lh);
xlabel('x','fontsize',FS);ylabel('y','fontsize',FS);zlabel('z','fontsize',FS);
y=get(gca,'YTickLabel');
set(gca,'YTickLabel',y,'fontsize',FS);
x=get(gca,'XTickLabel');
set(gca,'XTickLabel',x,'fontsize',FS);
% set(gca,'YTickLabel',y,'fontSize',12);
view(0,90);
grid on;
% axis off;
end
function LegendSize(lh)
set(lh,'units','pixels');
