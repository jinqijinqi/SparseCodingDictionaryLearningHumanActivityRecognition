function conf=Main_TranTest(nChunk,nHidden,FirstN)
%generate mesh
%Author: Jin Qi
%Date:   1/30/2014
...
JointPoint.Gamma=0.005;
JointPoint.MirrorGamma=0.03;%original
JointPoint.C=10; %
...
% print('-depsc2', [ResultPaperFigure num2str(i) 'Confusion']) ;
conf.numTest=length(selTest)
...
conf.NumSubset=numel(HistFeatList);
FS=12;
close all;
save([ResultPaperFigure num2str(i) 'ConfusionActionName.mat'], 'confus', 'conf','ActionName') ;
title(sprintf('Confusion matrix (%.2f %% accuracy)', ...
              100 * sum(diag(confus)/conf.numTest) ),'fontsize',FS/2) ;
PlotConfusion(confus,ActionName,ActionName,FS);
 title(sprintf('Confusion matrix (%.2f %% accuracy)', ...
              100 * sum(diag(confus)/conf.numTest) ),'fontsize',FS) ;
% print('-depsc2', [ResultPaperFigure num2str(i) 'Confusion']);
printpdf(gcf,[ResultPaperFigure num2str(i) 'Confusion']);
...
AverageAccuracy=AverageAccuracy/nTestTime;
AveragePrecision=AveragePrecision/nTestTime;
AverageRecall=AverageRecall/nTestTime;
xlabel(['Average Accuracy: ' num2str(AverageAccuracy) ' Average Precision: ' num2str(AveragePrecision)...
        ' Average Recall: ' num2str(AverageRecall)]);
fh=fopen([ResultPaperFigure 'AverageAccuracyPrecisionRecall.txt'],'a+');
fprintf(fh,'AverageAccuracy AveragePrecision AverageRecall NumChunk NumWordPerClass NumLargestCoef \r\n %f %f %f %d %d %d \r\n',AverageAccuracy,AveragePrecision,AverageRecall,nChunk,nHidden,FirstN);
fclose(fh);
end
% -------------------------------------------------------------------------
