...
nChunk=13;
nChunk=21; %for MSR Daily Activity
nChunk=23; %for MSR Daily Activity
% nChunk=19
% nChunk=25;
% nFrame=5;
%sampling step
...
TrainIndMSR=[1 3 5 7 9]; % training subjects for MSR dataset
h_Basis=figure(); % create a figure h1
h_Sample=figure();
for j=1:nSubject
    if strcmp(DataName,'Cornell')
        ...
        TrainInd=ismember(SubjectList,TrainIndMSR);
    end
...
%             end
%                  PlotVolume(Joint);
                 PlotSample(Joint,data,h_Sample);
        end
%         [BasisW WhitenV]=ICATrain(data);
%         [BasisW WhitenV]=DictionaryLearningBasedOnSC(NumAtom,data); %(1996)learning dictionary based on SC, too slow from "emergence of simple cell receptive field properties by learning a sparse code for natural images
        [BasisW WhitenV]=LearnDictL0(NumAtom,data);  % (2014)faster; from "L0 norm based dictionary learning for sparse coding" 
        title(['dictionary of ' Action]);
%         [icasig A W] = fastica(Patchdata, 'numOfIC',200);
        BasisMatrix(k)={BasisW};WhitenMatrix(k)={WhitenV}; 
%         BasisPatch(k)={W};[BasisW WhitenV]=LearnDictL0(NumAtom,data);
        Basis=(BasisW*WhitenV)';
%         figure(h_Basis);plot(Basis(:,1:10:100));title(['Basis for ' ActionName{TrainClass(k)}]);
    end
...
%  NormalizedData=ComputeDistanceAngle(NormalizedData); %using distance-Ang
...
function PlotSample(Joint,DataSample,h_Sample)
figure(h_Sample);
