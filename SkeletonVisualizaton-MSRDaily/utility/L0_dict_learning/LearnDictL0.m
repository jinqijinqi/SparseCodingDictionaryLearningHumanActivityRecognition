function [LearnedDict WhitenMatrix]=LearnDictL0(NumAtom,TrainData)
%---------------------------------------------------------------------------------------------------------------------------
%Author: Jin Qi
%Date:   11/25/2014
...
disp('initialization...');
%% 
[WhitenMatrix WhiteData]=PCAWhitenData(TrainData);% whiten data
Y=WhiteData;
...
% n_iter = 30;    % original; maxiaum number of iterations in dictionary learning
n_iter = 300;    % maxiaum number of iterations in dictionary learning
lambda = 6500;  % model parameter cotrolling sparisty regularization
% epsilon = sqrt(sz_patch^2) * std_noise * 1.1;   % target error for omp
epsilon = sqrt(size(Y,1)) * std_noise * 1.1;   % target error for omp
%% denoising
