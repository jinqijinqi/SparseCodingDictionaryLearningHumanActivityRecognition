function conf=Main_TranTest_MSRAction3D()
%generate mesh
%Author: Jin Qi
%Date:   1/30/2014
...
conf.ResultPaperFigure='./Result/PaperFigure/';
...
JointPoint.MirrorGamma=0.01;
JointPoint.C=10; %best value 89.70% for Joint point set "cornell"
JointPoint.lambda=0.01

% JointPoint.Gamma=0.035;
% JointPoint.MirrorGamma=0.3;
% JointPoint.C=10; %best value accuracy 88.889/350 "MSRAction3D" for all
% JointPoint.lambda=0.001

% JointPoint.Gamma=0.37;
% JointPoint.MirrorGamma=0.3;
% JointPoint.C=10; %best value accuracy 87.5.54/350  % MSRAction3D AS2
% JointPoint.lambda=0.015

% JointPoint.Gamma=0.3;
% JointPoint.MirrorGamma=0.3;
% JointPoint.C=10; %best value accuracy 87.6/350  % for MSRAction3D AS1
% JointPoint.lambda=0.015

% JointPoint.Gamma=0.5;
% JointPoint.MirrorGamma=0.3;
% JointPoint.C=10; %best value accuracy  97.29 for MSRAction3D AS3
% JointPoint.lambda=0.015


% JointPoint.Gamma=0.51;
% JointPoint.MirrorGamma=0.3;
% JointPoint.C=10; %best value accuracy 68  "MSRDaily3D" for all
% JointPoint.lambda=0.002

JointPoint.Gamma=0.3;
JointPoint.MirrorGamma=0.3;
JointPoint.C=10; %best value 71.6 accuracy  "MSRDaily3D" AS1
JointPoint.lambda=0.01
% 
JointPoint.Gamma=0.0003;
JointPoint.MirrorGamma=0.3;
JointPoint.C=10; %best value 71.6 accuracy  "MSRDaily3D" AS1
JointPoint.lambda=0.0001

% JointPoint.Gamma=0.7;
% JointPoint.MirrorGamma=0.3;
% JointPoint.C=10; %best value 81.25 accuracy  "MSRDaily3D" AS2
% JointPoint.lambda=0.015

JointPoint.Gamma=.5;
JointPoint.MirrorGamma=0.3;
JointPoint.C=10; %best value  85     accuracy  "MSRDaily3D" AS3
JointPoint.lambda=0.001
...
% sparse dictionary+direct projection
JointPoint.Gamma=1.05;
JointPoint.MirrorGamma=0.67;
JointPoint.C=10; %best value accuracy 70% Thr=0.02  for MSR Daily
JointPoint.lambda=0.00001
...
conf.numTrain=length(selTrain)
conf.numTestTrain=conf.numTest+conf.numTrain
conf.numClasses=length(classes)
conf.TestTime=ttest*conf.numTest/conf.numTestTrain
conf.TestTimePerVideo=ttest/conf.numTestTrain
conf.confus=confus;
conf.ActionName=ActionName;
conf.Accuracy=100.0 * sum(diag(confus)/conf.numTest);
conf.Precision=100.0*mean((diag(confus))'./(sum(confus)+eps));%column sum
conf.Recall=100.0*mean((diag(confus))'./(sum(confus')+eps));      %row sum
...
conf.AverageAccuracy=AverageAccuracy;
conf.AveragePrecision=AveragePrecision;
conf.AverageRecall=AverageRecall;
